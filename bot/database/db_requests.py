...
async def add_data(from_user, telegram_id, test, date, time, answer_time, scheduler_job_id):
...
async def get_sender(job_id: str):
...
async def get_product(offset: int):
    return await Product.all().offset(offset).limit(1).first()


async def get_catalog_length():
...
async def update_page(page: int, user_id: int):
    return await User.filter(user_id=user_id).update(page=page)


async def get_page(user_id: int):
    return await User.get_or_none(user_id=user_id).values_list("page", flat=True)


async def registration(user_id: int, name: str, date_of_birth: str, phone: str):
